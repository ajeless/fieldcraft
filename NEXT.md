...
- larger maps and boards should be possible without rendering every tile as a DOM control
...
1. `codex/board-viewport-layout`
   ...
   - Fix dense square-grid rendering, including the 64 x 64 line issue.
   ...
   - Avoid simply raising the grid-size limit until rendering no longer depends on one DOM control per tile.

2. `codex/space-and-scale-setup`
   ...
   - Keep the first UI small; the goal is to establish the shape without implementing every grid type.

3. `codex/editor-menu-bar`
